...
1.  **Check Plan Mode / Gates**:
    ...
    - Is a task waiting for review?
      - **Suggestion**: `/afx-work complete <task> "note"` (if you are the reviewer) or "Wait for human review."
...
5.  **Check Idle State**:
    - No active task, clean git state.
      ...
      - **Suggestion**: `/afx-work pick <spec>` (Pick up next task)
...
Next (ranked):
  ...
  2. /afx-dev code "continue"                     # Context-driven: Continue writing code
...
Next (ranked):
  ...
  2. /afx-work complete 7.4 "done"                # Context-driven: Mark task complete
  3. /afx-work pick docs/specs/{feature}          # Context-driven: Move to next task
  ──
  4. /afx-work status                             # Re-orient
...
Next (ranked):
  1. /afx-work pick <feature>                    # Context-driven: Pick next pending task
  ...
  ──
  4. /afx-work status                             # Check full project state
