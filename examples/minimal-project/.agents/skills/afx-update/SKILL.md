---
name: afx-update
description: Framework maintenance — check for upstream AFX updates and apply them safely with version comparison
license: MIT
metadata:
  afx-owner: "@rix"
  afx-status: Living
...
while [ $# -gt 0 ]; do
  case "$1" in
    --repo) REPO="$2"; shift 2 ;;
    --ref) REF="$2"; shift 2 ;;
    --skills-only|--no-docs|--no-claude-md|--no-agents-md|--with-gemini-md|--dry-run|--force|--yes)
      EXTRA_ARGS+=("$1")
      shift
      ;;
    *)
...
curl -fsSL "https://raw.githubusercontent.com/${REPO}/${REF}/afx-cli" \
