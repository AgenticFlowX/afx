...
Content area has no explicit max-width — stretches with viewport up to 1440px. Below 320px, layout remains single-column with horizontal scroll as a last resort.
...
- Title: `h2`, `font-weight: 600` — room name + address (e.g., "Spare room #2 — Sandringham flat")
...
- If `localStorage` is unavailable (private browsing, quota exceeded), fall back to light theme silently

## [DES-COLOR] Color Tokens

<!-- @see spec.md [FR-6] [NFR-1] -->
...
<!-- @see spec.md [FR-6] [FR-8] [FR-9] [NFR-4] -->
...
- When no approvals are pending, shows a no-op visual feedback (brief toast or chip flash) instead of navigating
