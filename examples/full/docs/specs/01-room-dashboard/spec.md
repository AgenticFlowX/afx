...
| FR-6 | Theme toggle      | Provide a light/dark theme toggle in the sidebar. Switching themes updates all CSS custom properties. Persist preference in localStorage. Falls back to light theme if localStorage is unavailable.                                                                                                                                     |
...
- [ ] Approval actions reflect the currently selected role (owner vs renter responsibilities)
...
None — all resolved. See journal RL-D003 for decision history.
